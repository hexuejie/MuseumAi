...
/* 全屏播放 */
//@property(nonatomic, strong) VideoFullScreenPlayViewController *videoPlayController;
...
// Load a movie
- (BOOL)load:(NSString*)filePath playImmediately:(BOOL)playOnTextureImmediately fromPosition:(float)seekPosition
{
//    (void)AudioSessionSetActive(true);
    ...
    if (mMediaState != NOT_READY && mMediaState != ERROR) {
        ...
        mediaURL = [[NSURL alloc] initFileURLWithPath:filePath];
...
- (BOOL)play:(BOOL)fullscreen fromPosition:(float)seekPosition
{
    ...
    if ((mMediaState != PLAYING ||
        ...
        if (requestedPlayerType == PLAYER_TYPE_NATIVE)
        {
            ...
            // own VideoFullScreenPlayViewController
            if(videoPlayController == nil)
            {
                videoPlayController = [[VideoFullScreenPlayViewController alloc] init];
            }
            rootViewController.definesPresentationContext = YES;
            videoPlayController.modalPresentationStyle = UIModalPresentationOverCurrentContext;
            ...
            videoPlayController.player = player;
//            [videoPlayController.player play];
            
            // Present the videoPlayController in the root view controller
            [rootViewController presentViewController:videoPlayController animated:YES completion:nil];
        ...
        */
        ...
        if (mLocalFile) {
            ...
            /*
            if (mPlayVideo) {
                // Start a timer to drive the frame pump (on a background thread)
                [self performSelectorInBackground:@selector(createFrameTimer) withObject:nil];
            } else {
...
- (BOOL)stop
{
    ...
    else if (mMediaState == PLAYING_FULLSCREEN)
    {
        ...
        // Dismiss the videoPlayController
//        [videoPlayController dismissViewControllerAnimated:YES completion:nil];
        
//        videoPlayController = nil;
    }
...
- (void)moviePlayerDidExitFullscreen
{
    ...
    CMTime currentTime = [videoPlayController.player currentTime];
...
- (void)doSeekAndPlayAudio
{
    NSLog(@"weizh doSeekAndPlayAudio");
