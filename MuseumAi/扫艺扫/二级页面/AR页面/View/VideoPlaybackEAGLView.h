...
static const int kMaxTargetsNum = 20;

// VideoPlayback is a subclass of UIView and conforms to the informal protocol
// UIGLViewProtocol
@interface VideoPlaybackEAGLView : UIView <UIGLViewProtocol, ZACGLResourceHandler, SampleAppRendererControl> {
@private
    // Instantiate one VideoPlayerHelper per target
    VideoPlayerHelper* videoPlayerHelper[kMaxTargetsNum];
    float videoPlaybackTime[kMaxTargetsNum];
...
/* ARModels */
@property(nonatomic, strong) NSArray<MUARModel *> *vars;
/* 是否正在下载 */
...
@interface VideoPlaybackEAGLView : UIView
#endif
